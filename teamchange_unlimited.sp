...
public Plugin:myinfo =
{
	...
	author = "Sheepdude, Franc1sco franug",
...
public OnPluginStart() 
{
	...
	HookConVarChange(h_cvarSpawnpoints, OnConvarChanged);
...
{ 
	...
	new String:char2[2] =  "1";
	if(interaction != -1)
		Format(char2, 2, "%c", g_cvarPenalty[interaction]);
	if(StringToInt(char2) > 0)
	{
		g_MapCount[client]++;
		g_RoundCount[client]++;
	}
	
	new bool:Access = StringToInt(char2) == 0 || (g_cvarImmunity && CheckCommandAccess(client, "teamchange_unlimited_immunity", ADMFLAG_GENERIC, true));
...
TeamChangeActual(client, toteam)
{
	...
	if(fromteam == TEAM_UNASSIGNED || fromteam != toteam)
	{
		...
		new m_iTs = GetTeamClientCount(TEAM_T);
		new m_iCTs = GetTeamClientCount(TEAM_CT);
		new imbalance = m_iCTs - m_iTs;
		...
		if(g_enabledspawnpoints && toteam == TEAM_T && m_iTs >= g_iTSpawns)
		{
			if(g_cvarChat)
				PrintToChat(client, "\x01\x0B\x04[SM]\x01 That team is full.");
			return;
		}
		else if(g_enabledspawnpoints && toteam == TEAM_CT && m_iCTs >= g_iCTSpawns)
		{
			if(g_cvarChat)
				PrintToChat(client, "\x01\x0B\x04[SM]\x01 That team is full.");
			return;
...
public OnConvarChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	...
	else if(cvar == h_cvarSpawnpoints)
		g_enabledspawnpoints   = GetConVarBool(h_cvarSpawnpoints);
